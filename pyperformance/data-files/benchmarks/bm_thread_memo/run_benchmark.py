...
def expected_total():
    """
    What the workers should add up to, computed once WITHOUT a cache.

    Deliberately uncached: a reference built through the same cache would
    agree with a broken cache. Calling normalise() on every token is the only
    way to catch a memo that returns a stale or wrong entry.
    """
...
def check_total(total):
    """
    Verify the combined worker output. Called once per benchmark call, never
    inside the timed region.

    A shared dict mutated without a lock can hand one thread another's value,
    and a per-thread cache keyed wrongly can return the wrong entry. Both show
    up here as a total that differs from the uncached answer.
    """
