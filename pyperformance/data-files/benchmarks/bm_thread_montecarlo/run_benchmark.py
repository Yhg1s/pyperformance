...
def check_estimate(counts, chunks):
    """
    Verify the sample count and the estimate, outside the timed region. Every
    chunk must report back with a count in range, and the estimate must sit
    within eight sigma of pi.
    """
...
def expected_own(chunks):
    """
    The exact count the per-thread-generator variant must produce. Seeding is
    deterministic, so an exact check is possible where a pi estimate would
    miss a chunk computed twice.
    """
