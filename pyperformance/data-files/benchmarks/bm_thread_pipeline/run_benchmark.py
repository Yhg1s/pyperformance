...
def check_consumed(results, threads):
    """
    Verify the pipeline drained correctly, outside the timed region. Every
    consumer must report back, every item exactly once, and the sum must
    match: a consumer transforming the wrong field still gets the count right.
    """
