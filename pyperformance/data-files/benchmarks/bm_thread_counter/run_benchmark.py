...
Both report the same total; only the frequency of touching the shared counter
differs.
"""
...
def check_counts(counted, total):
    """
    Verify the progress count and the parsed total. Called once per benchmark
    call, never inside the timed region.

    A lost increment is what an unsynchronised counter produces under
    threads. The total is checked alongside the count because a count can be
    right while the work was not: a chunk processed twice by one thread and
    skipped by another would keep the count correct and the sum wrong.
    """
