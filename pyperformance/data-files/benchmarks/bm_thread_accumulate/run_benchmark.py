...
The scoring work is identical in both; only the frequency of touching the
shared list differs.
"""
...
def check_results(results):
    """
    Verify the collected scores. Called once per benchmark call, never inside
    the timed region.

    Counting is not enough on its own: a lost append and a duplicated one
    cancel out in the total. Comparing the sorted values against the
    sequential answer catches a dropped record, a doubled record and a
    corrupted score, while staying order-insensitive, since the order results
    arrive in is nondeterministic.
    """
